...
function getReport(studentGrades) {
  ...
  for(var i = 0; i < studentGrades.length; i ++) {
    var obj = {}
    obj['classCode'] = studentGrades[i]['classCode'];
    obj['passed'] = [];
    obj['failed'] = [];
    result.push(obj)
  }
  var list = kelompok(result)
  var average = 70
  for(var a = 0; a < list.length; a ++) {
    for(var b = 0; b < studentGrades.length; b ++) {
      if(list[a]['classCode'] === studentGrades[b]['classCode']) {
        if(studentGrades[b]['score'] >= average) {
          list[a]['passed'].push(studentGrades[b]['name'])
        } else {
          list[a]['failed'].push(studentGrades[b]['name'])
        }
      }
    }
  }
...
function kelompok(array) {
  var obj = {}
  var result = []
  for(var i = 0; i < array.length; i ++) {
    var key = array[i]['classCode'];
    if(!obj[key]) {
      result.push(array[i])
      obj[key] = 'found'
    }
