...
function train(pokemon, candy) {
  ...
  if (pokemon["evolutionLine"] === undefined) {
    return "Congratulations, your " + pokemon["name"] + " grew to LV. " + newLevel + "!";
  } else {
    for (let i = 0; i < pokemon["evolutionLine"].length; i++) {
      if (pokemon["evolutionLine"][1]["level"] === newLevel) {
        return "Congratulations, your " + pokemon["name"] + " grew to LV. " + newLevel + "! And it evolved into " + pokemon["evolutionLine"][0]["name"] + " and " + pokemon["evolutionLine"][1]["name"];
      } else{
        return "Congratulations, your " + pokemon["name"] + " grew to LV. " + newLevel + "! And it evolved into " + pokemon["evolutionLine"][0]["name"]
      }
    }
  }
}
