...
function oddPairFirstLast(num) {
    if(num === 0) {
      ...
      var count = 0
      if(arr.length === 1) {
        var pasangan = arr.splice(0,1) + '1'
      } else {
        var pasangan = arr.splice(0,1).join('') + arr.splice(arr.length - 1).join('')
      }
      if(Number(pasangan) % 2 !== 0) {
        count = Number(pasangan)
      }
      var last = arr.join('')   
      return count + oddPairFirstLast(Number(last))
    }
}
